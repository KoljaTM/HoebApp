...
public class UpdateService extends BroadcastReceiver {
    ...
    public static final String CHANNEL_ID = "Erinnerung";
	...
	private void fireNotificationIfNecessary(final Context context) {
		...
		if (mediaCount > 0 && notificationSent < dueDate) {
			...
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
				NotificationChannel mChannel = new NotificationChannel(CHANNEL_ID, "Medien Fällig", NotificationManager.IMPORTANCE_DEFAULT);
				mChannel.setDescription("Erinnerung für fällige Medien");
